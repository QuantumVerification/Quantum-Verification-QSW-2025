...
from src.check_barrier import _sympy_to_z3_rec
from src.log import *
...
def constraints_Grover(N, M , err, num_samples, mu):
    ...
    A_dynamic, b_dynamic = generate_constraints(d_angles_G, c_index=0, right_hand_value=0, constraint_type='dynamic', initial_angles=d_angles)
...
def find_BC_grover(N_QUBIT, n_samples, err, mu, log_level=logging.INFO):
    ...
    if result.success:
        ...
        initial_conditions = [z3_barrier > gamma+0.000001, var_z3_dict[sym.re("phi")] >= low_i, var_z3_dict[sym.re("phi")] <= up_i]
        q = run_check(initial_conditions, 300)
        logger.info("Unsafe constraint...")
        unsafe_conditions = [z3_barrier < lambda_-0.000001, var_z3_dict[sym.re("phi")] >= low_u, var_z3_dict[sym.re("phi")] <= up_u]
        ...
        f_z3_barrier = _sympy_to_z3_rec(var_z3_dict, sym.poly(sym.re(sym.expand_complex(sym.poly(x_optimal[0]*(sym.Symbol("phi") + sym.Symbol("theta"))).as_expr()))).as_expr())
        z3_phi = var_z3_dict[sym.re("phi")]
        z3_theta = var_z3_dict[sym.re("theta")]
        dynamic_conditions = [f_z3_barrier - z3_barrier > delta+0.000001, z3_phi >= 0, z3_phi <= 2*np.pi, z3_theta <= theta+mu, z3_theta >= theta-mu]
